...
unwind[(rec) - [:CONTAINS]->(i)<-[:CONTAINS]-(r:Recipe) | i.name] as common_ings  with collect(distinct common_ings) as common_ingreds, count (distinct common_ings) as common_ingreds_count, ...

unwind(ingreds + original_ingreds) as all_ings with count(distinct all_ings) as all_ingreds_count , collect(distinct all_ings) as all_ingreds, …
